...
def get_by_class_saliency_dix(model, input,
                              label,
                              operations,
                              model_name='densnet',
                              layers=[12],
...
def heatmap_of_layer(device, input, interpolation_on_activations_steps_arr, interpolation_on_images_steps_arr,
    ...
    images = get_interpolated_values(torch.zeros_like(im), im, num_steps=0)
    ...
    for act in igacts:
        ...
        grads.append(calc_grads(model, act, device, label).detach() * act)
        act = act.detach()
